...
Resizing the window reflows the pane. Wrap width is chosen when a response is rendered, so answers that landed at different widths would otherwise sit side by side at different widths; when the new width holds still for a moment, every answer so far is re-rendered to it, at the close prompt as well as mid-run. The redraw clears the pane's scrollback along with the screen, which resets copy-mode position.
