...
@test "gemini: the default model is the flash alias, and it earns the token bump" {
    FAKE_BODY='{"candidates":[{"content":{"parts":[{"text":"x"}]}}]}'
    run_provider gemini.sh "hi" GEMINI_API_KEY=k
    [ "$status" -eq 0 ]
    grep -qF "models/gemini-flash-latest:generateContent" "$ARGV_FILE"
...
@test "openrouter: every routed model earns the token bump, whatever its id" {
    ...
    FAKE_BODY='{"choices":[{"message":{"content":"x"}}]}'
    local model
    for model in z-ai/glm-5.3-flash xiaomi/mimo-v2.5 deepseek/deepseek-r1 anthropic/claude-sonnet-5; do
        : > "$DATA_FILE"
...
@test "openrouter: COUNCIL_MAX_TOKENS still scales the bumped cap" {
    # The 8x arithmetic is tokens.bats' job; this proves the override reaches
    # the router's request now that no pattern sits between the two.
    FAKE_BODY='{"choices":[{"message":{"content":"x"}}]}'
