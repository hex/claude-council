...
test('defaults: pane on, collapse when done, no wake, no tool', () => {
  expect(paneOptions({})).toEqual({ isEnabled: true, collapsesWhenDone: true, wakesOnAsyncDone: false, offersTool: false })
})

test('declared values are taken, anything that is not a boolean is ignored', () => {
  expect(paneOptions({ pane: false, collapse_when_done: 'no', wake_on_async_done: true, council_tool: true })).toEqual({
    isEnabled: false,
