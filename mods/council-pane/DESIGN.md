...
- **Long field value**: a text field is one line, and Claude Code sizes a focused one to the whole pane, so in a framed form a longer value is cut at its end with `…` while it is typed; the counter under it gives the length and the roster shows the whole text once saved. The Enter hint is the bare `⏎` (an empty `submitLabel`) to leave the field more room.
