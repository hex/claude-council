...
- **Form**: each field's bold label on its own line with the field under it, then its help, one dim line cut at the end. The form has accent rules above and below instead of a frame, and spans the pane: Claude Code sizes a focused text field to the whole pane, so a frame or a label beside it cuts the text being typed. Given the whole width, the field wraps a long value onto following lines.
