...
- **Long field value**: a text field is one line, and Claude Code sizes a focused one to the whole pane, so in any frame it cuts a longer value at its end, where the cursor is. A value that does not fit gets one dim line under the field showing `…` and its end, from a word start. The Enter hint is the bare `⏎` (an empty `submitLabel`) to leave the field more room.
