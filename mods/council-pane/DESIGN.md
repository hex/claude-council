...
- **Templates table**: on the empty screen only, the table component with two columns, `TEMPLATE` and `USE WHEN`; the name is a plain Button, the use-when dim. It reads as the roster to come, so picking a template feels like picking a row.
