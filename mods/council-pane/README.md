...
A specialist is a Codex agent that writes code for you on its own model, in its own git worktree. You name it, pick its model, say when it fits, and optionally name skills it follows. Claude offers one when a task matches and starts it only once you ask or agree. The start itself opens no dialog; only a finish asks you first.

Run `/specialists` to set them up. It opens a screen with your specialists as a table in an orange frame: name, model, effort and use-when, one line each; a long use-when is cut with `…`, and the skills stay in the form. The header has a neutral grey fill and dim bars separate the columns. Efforts get warmer as they rise, and every other row is shaded; the row you edit, and the one under the pointer, take the selection colour. Pick one to edit it in the form below, or add a new one. You type the name, the use-when and the skills, and pick the model and the effort from lists:
...
| Name | Use when |
|---|---|
| `bug-fixer` | A bug reproduces, the expected behaviour is clear, and the fix stands apart from the current work. |
| `ci-fixer` | A CI job fails on a named revision and the fix belongs in code, config or tests, not in the CI provider. |
| `refactorer` | A behaviour-preserving restructure is named: extract, rename, move, split a module, or remove duplication across three or more sites. |
| `test-writer` | The task is to add or extend tests for existing behaviour that has a spec, a documented contract, or a confirmed expected output. |
| `test-pruner` | Tests in a named area duplicate stronger tests, re-assert the implementation, or keep test-only seams alive, and need pruning. |
| `docs-updater` | Docs must be updated or restructured to match code that already changed: README, reference pages, CLI help or config docs. |
...
[
  ...
  {"name": "mig", "model": "gpt-6-luna", "effort": "high", "when": "schema changes",
   "skills": ["test-audit", "web-perf"]}
]
```

Each entry needs `name`, `model` and `when`; `effort` and `skills` are optional, `"enabled": false` switches it off, and the mod refuses any other key by name. The name is lowercase letters, digits and dashes. The model goes to `codex exec -m` as written. `effort` must be one of the levels Codex offers for that model; without it, your own Codex default applies. `when` is what Claude matches tasks against, one line of up to 200 characters. `skills` is a list of up to 8 skill names, each lowercase letters, digits, dots, dashes or underscores, none twice; a Save refuses one that no folder holds. Setting the whole list by hand, as `/config claude-council.specialists=[...]`, goes through the same checks as a Save, and a refusal names the entry and the reason. While a Remote Control connection may be live, Claude Code hides that reason and prints `Couldn't save this setting (detail withheld on this connection).` instead, and the stored list is unchanged. The check asks Codex for its catalog, so it takes about a second. At session start the mod skips an entry that still does not parse, for example one written straight into `settings.json`, and the session log says which one and why. If the mod cannot read the stored list itself, the screen says why, and Save and Remove stay off until you fix it with `/config`. The mod registers `mcp__claude-council__specialist` in every session; with no specialists it only offers to set one up, and follow-ups, results and finishes for a run that is still open.

The tool takes five calls. Only a finish opens a dialog:

- Start, `{specialist, task}`: reads the specialist's skills, creates the worktree from `HEAD` and starts round 1. A skill that is no longer installed refuses the start before the mod creates anything. When you have uncommitted changes, the reply says the worktree lacks them.
...
- Setup, `{setup: {name, model, effort, when, skills}}`, every field optional, `skills` as names separated by commas: opens the setup screen with those fields filled in. The screen saves nothing until you press Save. With no specialists set up, this is the only call the tool takes.
...
To carry gitignored local files into a new specialist worktree, list their relative paths in the repository root's `.worktreeinclude`, one per line. Blank lines and lines starting with `#` are ignored; missing paths are skipped. A start is refused when an entry is absolute, contains `..`, meets a symlink, or names a path git does not ignore, since the round's commit would carry it into the merge. This repository lists `.claude/types` so the pane's TypeScript check can use locally generated plugin types.
