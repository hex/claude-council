...
Network access is on inside the Codex sandbox. The worktree limits where a specialist can write, not what it can send: a specialist can read the worktree and reach the network, so it can send what it reads anywhere. Codex runs with your own `~/.codex/config.toml` and global instructions, so your extra writable roots and house rules apply to specialists too. The MCP servers listed in `config.toml` do not: a round turns them off, since none serve the task and each would start with every round. Servers a Codex plugin provides still start. The mod forwards no API keys; Codex uses its own login.
