...
The mod ships no specialists. While you have none, the screen offers six templates under the empty table, one `Use <name>` button each. Each opens a new form with the template's name, use-when and instructions, on the first model Codex lists. Pick the model and effort you want and press Save. The screen saves nothing before that, and over unsaved changes the button asks first. The templates name no model because the models depend on your Codex account.

| Name | Use when | Instructions |
|---|---|---|
| `bug-fixer` | A bug reproduces, the expected behaviour is clear, and the fix stands apart from the current work. | Reproduce the failure first and name the failing test or command. Find the root cause before changing code. Make the smallest fix at the owning code. Show the reproduction failing before the fix and passing after it. Never weaken or skip a test to make it pass. Report the commands you ran and their results. |
| `ci-fixer` | A CI job fails on a named branch or commit and the fix belongs in code, config or tests, not in the CI provider. | Work from the exact SHA, run and job given. Read the failed log once. Classify the failure as product, test harness, environment or credential before changing anything. Reproduce in CI's OS and runtime when you can. Never retry until green, skip, or weaken a check. Report the class, the cause and the proof. |
| `refactorer` | A behaviour-preserving restructure is named: extract, rename, move, split a module, or remove duplication across three or more sites. | Keep behaviour identical. Read every call site before editing; never edit by regex. Update all callers in the same change and delete the old path: no aliases, shims or re-exports. Run the existing tests before and after, and do not change them unless a test asserts the old structure. Report what moved and the test results. |
...
| `test-pruner` | Tests in a named area duplicate stronger tests, re-assert the implementation, or keep test-only seams alive, and need pruning. | For each candidate record what it can catch, the stronger test that still covers it, and the seam its removal frees. Delete or consolidate only candidates with that record, and remove the test-only exports they kept alive. Never delete a test that fails on the baseline; report it as a possible bug. Report kept, fixed, merged and deleted tests. |
| `docs-updater` | Docs must be updated or restructured to match code that already changed: README, reference pages, CLI help or config docs. | List every fact the old doc states, then keep, move or delete each one, citing the source that proves a deletion. Verify commands, flags, defaults and limits against code or --help, never the old doc. Change no code. After editing, compare old and new and report where moved or removed facts went. |
