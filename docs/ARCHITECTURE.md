...
| `COUNCIL_THEME` | auto-detected | Force pane emphasis palette: `light` / `dark` (else OSC 11 query; `COLORFGBG` only asserts `light`, never `dark` since it goes stale; otherwise attribute-only emphasis that inherits the foreground) |
