...
| `/config` rows `pane_host`, `collapse_when_done`, `wake_on_async_done`, `council_tool` | ask, on, off, off | The council-pane mod's settings, read from the plugin's `userConfig`; `mods/council-pane/README.md` describes each |
