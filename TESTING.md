...
| `transcript-digest.bats` | 20 tests | session JSONL to markdown: the four-clause human-turn filter (a tool result, a cross-session peer message carrying another Claude's prose, and a compact summary all excluded), assistant replies interleaved in file order, thinking blocks and their signatures dropped, `--turns last:N` windowing, and a bare session id refused with the lookup named rather than performed |

**Total: 658 tests** across 28 `.bats` files.
