...
| `router-seats.bats` | 12 tests | `OPENROUTER_MODELS` splitting the one router script into numbered seats: per-seat `get_model` (roster entry, `<SEAT>_MODEL` override, out-of-range reporting `unknown` rather than a neighbour's id), discovery replacing rather than joining the single seat, `provider_script_path` mapping every seat back to one script, per-seat vision opt-in, colour/emoji not falling to the unknown-provider default, and an end-to-end run asserting each seat both is labelled with and actually queried its own model |
...
**Total: 600 tests** across 26 `.bats` files.
